...
Research Assessments are local to this Grant. Return material source-grounded routes that became refuted or blocked during this Grant, but do not re-emit Assessments from an earlier Grant. A refuted Assessment needs the source-visible control or evidence that disproves it. A blocked Assessment needs one or more exact unresolved facts. Assessments are not Coverage units, a work queue, or proof that the Target is safe. Every report must also include a Grant-local evidenceSummary. Its examinedAreas must name source areas actually inspected and cite source evidence; its unexaminedAreas must name material areas not inspected during this Grant. Neither list is a Harness work queue or proof of coverage completion.
