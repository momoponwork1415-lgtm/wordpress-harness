...
const verificationHypothesisOutputSchema = sourceBoundHypothesisSchema.omit({
  kind: true,
  schemaVersion: true,
  route: true,
});

const depthEvaluationDispositionSchema = z.discriminatedUnion("action", [
  z.strictObject({
    proposalId: digestSchema,
    action: z.literal("request-verification"),
    hypothesis: verificationHypothesisOutputSchema,
    reason: boundedTextSchema,
  }),
  z.strictObject({
    proposalId: digestSchema,
    action: z.literal("schedule-missing-link"),
    gapId: digestSchema,
    reason: boundedTextSchema,
  }),
  z.strictObject({
    proposalId: digestSchema,
    action: z.literal("retain-route"),
    reason: boundedTextSchema,
  }),
  z.strictObject({
    proposalId: digestSchema,
    action: z.literal("close-route"),
    reason: boundedTextSchema,
  }),
  z.strictObject({
    proposalId: digestSchema,
    action: z.literal("block-route"),
    reason: boundedTextSchema,
    reopenWhen: boundedTextSchema,
  }),
]);

export const depthRootEvaluatorOutputSchema = z.strictObject({
  kind: z.literal("depth-root-evaluator-output"),
  schemaVersion: z.literal(1),
  dispositions: z.array(depthEvaluationDispositionSchema).min(1).max(32),
});

const depthIterationActionSchema = z.discriminatedUnion("kind", [
  z.strictObject({
  ...
  z.strictObject({
    ...
    proposal: chainProposalRefSchema,
    gap: criticFrontierGapRefSchema,
    reason: boundedTextSchema,
  }),
  z.strictObject({
...
export const depthIterationDecisionSchema = z.strictObject({
  ...
  attempt: depthEvaluatorAttemptRefSchema,
  actions: z.array(depthIterationActionSchema).min(1).max(32),
});

export const depthIterationDecisionRefSchema = z.strictObject({
  kind: z.literal("depth-iteration-decision"),
  schemaVersion: z.literal(1),
  id: digestSchema,
  digest: digestSchema,
...
export const depthEvaluationIncompleteSchema = z.strictObject({
  ...
  synthesis: chainSynthesisRefSchema,
  critique: adversarialCritiqueRefSchema,
  attempts: z.array(depthEvaluatorAttemptRefSchema).max(1),
  reason: z.enum([
    "evaluator-failed",
    "invalid-output",
    "result-mismatch",
...
export const semanticDepthEvaluationInputSchema = z.strictObject({
  kind: z.literal("evaluate-depth-research"),
  schemaVersion: z.literal(1),
  registry: z.strictObject({
    ref: approachFamilyRegistryRefSchema,
    value: approachFamilyRegistrySchema,
  }),
  synthesis: chainSynthesisSchema,
  critique: adversarialCritiqueSchema,
});

export type DepthRootEvaluatorOutput = z.infer<
  typeof depthRootEvaluatorOutputSchema
>;
export type DepthIterationDecision = z.infer<
  typeof depthIterationDecisionSchema
>;
export type DepthIterationDecisionRef = z.infer<
  typeof depthIterationDecisionRefSchema
>;
export type DepthEvaluationIncomplete = z.infer<
  typeof depthEvaluationIncompleteSchema
>;
export type SemanticDepthEvaluationInput = z.infer<
  typeof semanticDepthEvaluationInputSchema
>;
export type SemanticDepthEvaluationResult =
  DepthIterationDecision | DepthEvaluationIncomplete;

export interface SemanticDepthEvaluation {
  evaluate(
    input: SemanticDepthEvaluationInput,
  ): Promise<SemanticDepthEvaluationResult>;
}

export interface OpenSemanticDepthEvaluationOptions {
...
type FailureReason = DepthEvaluationIncomplete["reason"];
...
function validateInput(input: SemanticDepthEvaluationInput) {
  const synthesisRef = {
  ...
  if (
    ...
    !same(input.synthesis.manifest, input.critique.manifest) ||
    !same(synthesisRef, input.critique.synthesis)
  ) {
    throw new Error("Depth Evaluation input binding mismatch");
  }
...
function attemptPlan(
  ...
  options: OpenSemanticDepthEvaluationOptions,
): Extract<AttemptPlanV2, { role: "root-evaluator" }> {
  const attemptId = `depth-root-evaluator:${sha256Digest({
  ...
  const plan = attemptPlanV2Schema.parse({
    ...
    prompt: [
      ...
      "A surviving route may be retained or requested for fresh Independent Verification. A Verification request must state its causal identity, exact attacker premise, impact, unresolved evidence, falsifier, and next experiment; the Harness derives route anchors from the Chain Proposal.",
  ...
  if (plan.role !== "root-evaluator") {
    throw new Error("Depth evaluator materialized with another role");
  }
  return plan;
}
...
function resolveOutput(
  ...
  attempt: z.infer<typeof depthEvaluatorAttemptRefSchema>,
):
  | { readonly kind: "completed"; readonly value: DepthIterationDecision }
  ...
  const actions: z.infer<typeof depthIterationActionSchema>[] = [];
  ...
  return {
    kind: "completed",
    value: depthIterationDecisionSchema.parse({
      ...identity,
...
export function referenceDepthIterationDecision(
  value: DepthIterationDecision,
): DepthIterationDecisionRef {
  const decision = depthIterationDecisionSchema.parse(value);
  return depthIterationDecisionRefSchema.parse({
    kind: "depth-iteration-decision",
    schemaVersion: 1,
    id: decision.id,
...
export function openSemanticDepthEvaluation(
  options: OpenSemanticDepthEvaluationOptions,
): SemanticDepthEvaluation {
  return {
    evaluate: async (rawInput) => {
      const input = semanticDepthEvaluationInputSchema.parse(rawInput);
      const refs = validateInput(input);
      const plan = attemptPlan(input, refs, options);
      const result: AttemptExecutionResult =
        await options.modelExecution.run(plan);
      const attempts: z.infer<typeof depthEvaluatorAttemptRefSchema>[] = [];
      ...
      if (result.value.schemaVersion === 2) {
        ...
        if (
          ...
          const attempt = depthEvaluatorAttemptRefSchema.parse(result.ref);
      ...
      return depthEvaluationIncompleteSchema.parse({
