...
>
> **Harnessは権限、証拠、隔離、制限を管理し、エージェントは探索上の判断を担う。**

探索は一つの連続した流れです。AIがソースコードに基づく具体的な次の手を持つ間は、同じチェックポイントから人間の操作なしで続けます。調べる価値のある経路がなくなった時だけ停止し、候補があれば人間の採否判断へ渡します。Harnessは担当エージェント数、探索の段階、深さ、脆弱性の種類、読むファイル、固定評価表を決めません。未認証SQLインジェクションや格納型XSSは、RCEへ発展しなくても候補です。
...
この通常経路は、人間が承認した対象、実行直前の観測、対象の受入情報、キャンペーン方針、WordPress本体などの依存ソース、脅威の前提、プログラム上の探索境界を一つの要求へ結び付けます。正確なスキーマは[`src/target-intelligence/approved-target-campaign/contracts.ts`](src/target-intelligence/approved-target-campaign/contracts.ts)、探索スキーマとプロンプトのダイジェストを作る処理は[`src/research/agent-led/contracts.ts`](src/research/agent-led/contracts.ts)にあります。`--dependency-source`のマウント名は要求と一致させます。実行環境は封印済みの`agentRuntimeProfile.kind`から選びます。探索試験ではGrokを優先し、利用できなければキャンペーンを`incomplete`として残します。

非公開の対象ソース、プロンプト、プロバイダー出力、認証情報、ペイロード、実行記録、未公開の発見事項はGit外へ置きます。対象のオートロード、Composerスクリプト、WordPressの起動処理をホスト上で実行しません。
