...
**Transport適格性記録（Transport Eligibility Receipt）**:
...
**Provider認証保管庫（Provider Credential Store）**:
...
**実戦Campaign（Prospective Campaign）**:
...
**根拠状態（Evidence State）**:
Surface Mapのnodeまたはrelationを、固定sourceから直接確認した`observed`、根拠から導いた`inferred`、接続を確定できない未解決（`unknown`）のいずれかとして表す区分。不明を観測事実へ昇格させない。
...
**Programme適格性判定（Programme Disposition）**:
...
**未完了Campaign（Incomplete Campaign）**:
