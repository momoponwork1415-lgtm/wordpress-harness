...
Target Intelligenceが受け渡した入力を受け取り、調査、AIによる自律継続、候補の人間審査、Candidate Verification Requestと進捗の記録を担当する。対象選定、ソースの取得方針、実環境での検証、programme scope、外部送信は担当しない。
...
| 用語 | 意味 |
| --- | --- |
| **Campaign** | 対象、依存ソース、指示文、実行方式、権限、安全上限を固定した、再開可能な一件の調査。 |
...
| **Discovery** | ソースに基づく仮説、候補、保留中の手がかりを更新する探索部分。具体的なsource-bound next actionがある`continue`は同じCheckpointから自動継続し、途中に動的検証を割り込ませない。 |
...
| **Independent Research Trial** | 以前の結果やCheckpointを入力せず、freshなCampaignとして始める外側の一試行。同じCampaign内の継続Native Run、通信retry、出力形式の再試行は別Trialに数えない。 |
| **Evaluation Sweep** | 固定した評価対象集合の各TargetへIndependent Research Trialを一回ずつ行う評価単位。同じTargetへの複数Trialや一Campaign内の複数Native Runとは区別する。 |
| **Research Report** | 一回のNative Runが返す、調べた領域と未調査領域のsource-backed summary、Research Assessment、候補、保留中の手がかり、継続または停止の提案。 |
...
| **Coverage** | Research Recordからread-onlyに導出する、調べた領域、未調査領域、残るsource-bound next actionの表示。別のmutable ledgerや探索完了の証明ではなく、発見件数や対象の安全性も意味しない。 |
| **Interruption** | 安全上限、提供元、権限、不正な出力などにより、判断を完了できなかった記録。 |
...
| **Research Record** | 入力、実行結果、非公開記録への参照、Human Candidate Review、Candidate Verification Request、中断を追記する正本。Coverageはこの記録から導出し、会話本文や作業メモ、内部の呼び出し順を業務上の状態にしない。 |

## 守るべき区別

- Root AIの`continue` / `stop`と、`stop`後のHuman Candidate Reviewを分ける。承認済みCampaign内のsource-boundな`continue`にrunごとの人間承認を要求しない。
...
- 同じCheckpointから継続するNative Run、freshなIndependent Research Trial、評価対象全体のEvaluation Sweepを分ける。

権限・隔離・安全上限・失敗時の設計原則は[探索設計](docs/RESEARCH-DESIGN.md#trust-and-versioning)、現在のInterfaceと回帰テストは[コードベース案内](docs/CODEBASE-GUIDE.md#research-campaigns)を参照する。
