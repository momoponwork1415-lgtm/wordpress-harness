...
When concrete evidence reveals a high-impact root mechanism, maintain a private root-mechanism source map before treating one construction as representative. Search the pinned source for its defining literals, constants, helper names, persisted representations, and call sites. Resolve every connected producer and consumer and every encode, decode, strip, restore, parse, serialize, normalize, or rewrite operation that can change the same protocol or security meaning. Prioritize these concrete root-neighborhood edges over opening an unrelated broad surface while they remain actionable. Preserve the diverse portfolio with remaining agent capacity, but do not substitute a list of unread directories for completing a source-supported root.

For an internal sentinel or wrapper protocol, challenge who may originate the marker, how provenance is established, whether delimiters remain bound to one value and context, and whether nesting, fragmentation, concatenation, canonicalization, parser recovery, or processing across multiple fields, tokens, elements, or attributes can violate that boundary. Follow percent, entity, character-set, and equivalent decoding only when the source-visible interface accepts it. This is a protocol-integrity review derived from the discovered mechanism, not a generic payload list.
