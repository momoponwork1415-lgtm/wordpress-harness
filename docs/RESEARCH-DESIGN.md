...
**危険な処理の入口だけに最適化せず、壊れたセキュリティ上の意味を探します。**
...
候補は、まず次の適格条件を満たす必要があります。

- 正確な製品と版を特定でき、変更されないソースとファイル一覧を固定できる。
- 宣言したプログラムの対象範囲で探索と後続検証が許され、報告経路がある。調査目的だけで扱う場合は、その理由を明示する。
- 隠れた権限や特殊な外部環境を仮定せず、通常構成と必要な依存製品・外部設定を説明できる。
- 起動直前に、版、ソース、プログラムの対象範囲を再確認できる。

適格な候補どうしは固定点数で順位付けせず、次の観点をまとめて比較します。

| 観点 | 確認すること |
| --- | --- |
| 影響の上限 | アカウント所有権、権限、ファイル、データベース、決済、管理者画面、プラグイン導入、サイト全体の状態へ影響し得るか |
| 攻撃者からの近さ | 未認証訪問者、フォーム送信者、顧客、購読者、Webhook送信者など、通常運用に実在する位置から届くか |
| 境界の豊かさ | 外部入力からローカル権限、公開コンテンツから管理者画面、URLから導入処理など、安全上の意味が切り替わる境界があるか |
| 再発の事前分布 | 公開履歴の件数、観測期間やソース規模に対する密度、CVSS最大値と分布、High / Critical件数、直近性から、広く安全上重要な面を持つと考えられるか。単発の高得点と重大記録の反復を分け、個別の既知経路は探索へ渡さない |
| 運用可能性 | 正確なソース、通常構成、依存製品、検証用アカウントやコールバックを後で再現できるか |

Patchstack VDPなど特定プログラムを優先する場合は、公式の対象範囲と報告経路を候補集合の条件にします。プログラム名や対象となる製品面は探索へ渡せますが、既知報告の題名、影響箇所、修正差分、PoCは対象情報の非公開記録に留めます。
...
DeepSeekを使う既知事例の評価は、同じ対象と条件でfreshなCampaignを最初から3回起動する`pass@3`とし、3試行のCandidateを出所付きの和集合として評価します。試行数を結果で変えると再現率の測定が歪むため、評価では条件分岐させません。同じ主張らしく見えるCandidateも自動で同一化せず、各試行の記録を保持します。

未知対象の本番探索は、承認時に同一条件のfreshな3試行と全体の安全上限を先に固定した上で、初回だけを起動する条件付き3試行を標準とします。初回がモデル上完了しCandidateを一件以上返した時だけ、残り2試行を互いに並列で起動します。Candidateの種類、深刻度、確信度は起動条件にせず、HarnessはCandidateの存在だけを見ます。残り2試行へ初回のCandidate、報告、Checkpointを渡さず、各試行の独立性を維持します。初回がCandidateなしで正常完了した場合は残りを起動せず、初回が`incomplete`、実行中、または結果を読めない場合はCandidateなしへ読み替えず停止します。これは費用配分の運用方針であり、固定3試行による`pass@3`の評価値とは別に表示します。
...
比較表示は、既存のCampaign表示、Native Receipt、候補の動的検証、プログラム対象範囲の記録から読み取り専用で導きます。二つ目の更新可能な台帳、`ResearchStrategy.execute`は追加しません。Campaign内部の自律継続は`ResearchCampaigns`だけが所有します。正確な試行集合と、全体の実行時間・run数への人間の承認を先に求めます。Operationsの起動ゲートは、承認済み集合から初回または後続2試行を選ぶだけで、探索判断、候補採否、動的検証、外部行動を代行しません。
