...
Claude Code用native bridgeは、公式MCP TypeScript SDKのAttempt-local loopback HTTP server、private bearer config、exact tool allowlist、`dontAsk`、connection fail-closedとして同日に実装した。fake providerのexact tool inventoryと`search -> read` Behavior Test、およびsubscription認証済みClaude Code 2.1.258の合成live probeを通過した。`Campaign Run Plan`はTarget-boundなSource Tool Policy refとquery ceilingを固定し、production Finder materializerは対応するPolicy本文、digest、Target bindingを検査して各`Attempt Plan`へ結合する。CampaignからFinderまでのbindingは`CampaignRunner.run(plan)` Behavior Testで固定した。Brizy、Appointment Booking Calendar、TranslatePressのprivate実Target Campaignをtool付き3並列で再実行し、Source Evidence queryとterminal resultをCASへ保存した。`symbol`、`graph`、complete Attempt Receiptは後続sliceである。

同日、Finder terminal outputへ任意の`Route Fragment Proposal`を追加した。完全なHypothesisへ届かないprimitiveを、攻撃者前提、前提条件、消費・生成値、state transition、source anchor、unknown、falsifier、次調査へ構造化し、Work LeaseのHypothesis上限と同じ件数上限でFinder Attempt Resultとprivate CASへ保存する。旧v1 outputとのreplay互換のためfieldは省略可能である。現時点ではModel Executionによるschema検査と保存までであり、ExplorationはProposalをTarget inventoryへ再拘束せず、Wave barrierで取込・deduplicate・Chain Synthesisもしない。
