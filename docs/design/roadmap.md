...
North Starは、oracle-freeなprospective Campaignでhigh-impactなbroken security semanticsを発見し、独立Verificationで実証する能力である。RCEやsite-wide compromiseは最上位impactだが唯一の成功条件ではない。Unauthenticated SQL injection、意味的に深いStored XSS、account takeover、privilege escalation、arbitrary file operation、object injection等も独立した重大Findingとして扱い、high-impactへ伸びるRoute FragmentはDepth Escalationの入力にする。
...
flowchart TB
    ...
    m4["4. Target Intelligence"]
    m5["5. Breadth and cost optimization"]
...
## 5. Breadth and cost optimization
