...
Work LeaseからAttempt Planを作る処理はCampaign Control implementation内部へ置く。Target Snapshot、Surface Map、PHP Program Indexから、Leaseが所有するFocus Areaに必要なsource range、observed relation、unknown、Strategy、budgetを`Analysis Unit@v1`として決定的にrenderする。directed Strategyが二次sourceを読めた場合、そのsourceにある低頻度literal参照を最大1件、共有hookの別登録元を最大1件、一段だけ同じfile/byte上限内へ昇格する。model自由文からpathを抽出せず、昇格sourceから再帰展開せず、`wildcard`には適用しない。Unitは実際に渡すpath、file digest、range、byte量、選択理由を持つが、選択理由をreachability evidenceとして扱わない。
