...
Git外のBrizy 2.8.11/2.8.12 snapshotでは、同じproduction materializerで各3 Leaseを外部modelなしに構築し、全Unitが8 files、650 KB以下へ収まることを確認した。Focus correction後は両版とも外部AJAX entryへ`entry-forward`と`wildcard`を重ね、別のcode-execution primitiveを`sink-backward`へ置いた。Wildcardは広いSurface Map標本を先に選ぶため、directed Strategyとは別のsource集合になった。これはFocus順位、source選択、bindingのcharacterizationであり、attacker reachability、脆弱性発見、またはpatched negativeの実証ではない。
