...
現行PHP Program Indexは`source-mapping/php-program-index`へ移し、Research contextの公開Interfaceから隠した。ただしSurface Mapはまだ生成しない。次のmapping behaviorでは[Source mapping seam](source-mapping-seam.md)の`build`だけをtest surfaceとし、observed・inferred・unknown（未解決）の根拠状態、非PHP asset coverage、immutable revision、Context Requestをred-greenで追加する。静的なinitial mapとsource-only revisionを先に成立させ、実戦でdynamic mapping gapが出てから`runtime-revision`とRuntime Observationを追加する。既存PHP Program Index testはSource Mapping内部のmodule seamから観測できるbehaviorを保護する。
...
現行実装にはExploration behaviorがない。[Exploration seam](exploration-seam.md)の`decide(ExplorationDecisionInput)`だけをtest surfaceとし、最初に固定Surface Map fixtureからFocus Area、LaneとStrategyの別軸割当、source-bound Hypothesis受理、stable rankingを一つのpure sliceで実装する。その後、minority route保持、Chain Synthesis、Gap Review、Runtime Observation requestをそれぞれ独立したred-green sliceとして追加し、UIや運用自動化を先行させない。
