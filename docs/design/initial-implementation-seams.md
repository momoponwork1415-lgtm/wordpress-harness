...
現行Explorationは`decide({ kind: "bootstrap", ... })`だけを実装し、固定Surface Mapから非重複ownerを持つFocus Area、有限Work Lease、LaneとStrategyの別軸割当、model-family constraint、Wildcard枠をpureかつ決定的に計画する。provider process、Research Ledger、Labは呼ばない。Finder output取込、source-bound Hypothesis、minority route保持、Chain Synthesis、Gap Review、Runtime Observation requestを後続の独立したred-green sliceとして追加し、UIや運用自動化を先行させない。
