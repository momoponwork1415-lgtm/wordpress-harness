...
| 内容 | ソース | 現在の関係 |
| --- | --- | --- |
| base Prompt | [WP2Shell-derived](../prompts/wordpress-plugin-research-v7.md) | `research-prompt-set.ts`が唯一のcanonical Prompt Set IDと本文digestをbindする。公開原文の動的multi-agent、十分に異なる探索経路、複数round、遅い知見の交差、敵対的二重確認、Rootの反復統合を維持し、課題固有のpositive oracle、RCE / `/flag`強制、最低6時間だけを除く。CLIは正本本文を渡し、sealed digestを検査する |
