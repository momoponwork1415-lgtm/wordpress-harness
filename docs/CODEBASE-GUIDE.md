...
- **Interface:** production compositionは`WordfenceIntelligenceRefresh.run / inspect`。`openSqliteHostPrivateCredentialBroker({ databasePath })`を`credentialBroker`へ渡すと、host-private SQLite brokerから実行できる。既存のfixed-main legacy indexを一度だけ移行する場合だけversioned `legacyStorageAdoption`を明示する。local indexは既存の`WordfenceIntelligence.refresh / inspect / aggregate / inspectKnownRecords`と`KnownRecordAccessAuthorizationProvider`を維持する。
- **Invariants:**
  ...
  - **Index and raw artifacts:** production indexはDatabase constructorより前にowner-only parentとDB / WAL / SHMを`O_NOFOLLOW`で検証し、open前後のdevice / inodeをcached instanceへbindする。新規storeと明示したfixed-main legacy adoptionだけがowner-controlled pathをsecure化できる。adoptionはpinned readonly FDから旧Wordfence schemaとschema metadataを確認してからchmod / DDLへ進み、任意SQLiteやmarker-only DBを変更しない。initialized storeのdriftはfresh compositionでも修復しない。raw CASはpinned directory配下の同期済みowned tempからatomic no-clobber installし、winnerをowner、regular file、mode `0400`、全bytesへ照合してparentをfsyncしてからSQLite参照をcommitする。
  - **Storage format:** 新規production storeとverified legacy migrationはschema version、固定5分Attempt leaseを持つv1 production-storage marker、order `0/0`、allocator `0`を同じtransactionで作る。normal reopenはmarker、完全なtable / index schema、schema metadata、order、allocatorをreadonlyで検証してから開き、欠落をlegacyと推定せずDDLで補わない。
