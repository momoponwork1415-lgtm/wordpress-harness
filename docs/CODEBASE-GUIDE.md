...
Manual local-directory Target Intakeは実装済みである。Target選定、archive acquisition、Intake PacketからCampaign preparationへのcontext handoff、Human Review PacketからHuman Confirmationまでのproduct pathは未実装である。

## Implementation index

| Capability | Status | Public or owner Interface | Source | Behavior Tests | Design |
| --- | --- | --- | --- | --- | --- |
| Manual Target Intake | partial; local directoryのraw-byte capture、stable manifest、WordPress.org identity / header version照合、link / hardlink / path collision / quota拒否、曖昧なmain fileのdeferred、CAS-first Packet / Receipt。archive acquisitionとCampaign handoffは未実装 | `TargetIntake.intake` / `openLocalDirectoryTargetIntake` | [`target-intelligence/acquisition/`](../src/target-intelligence/acquisition) | [`local-directory intake`](../tests/target-intelligence/local-directory-target-intake.test.ts) | [Target Intake seam](design/target-intake-seam.md) |
