...
- **目的:** pass@Nの各試行を以前の結果やCheckpointを持たない通常のResearch Campaignとして起動し、同一条件の独立Trialだけをread-only comparisonへ渡す。DeepSeekの通常評価ではApprovalへ3 Trialを定義し、比較側で出所を保ったCandidateの和集合を表示する。
