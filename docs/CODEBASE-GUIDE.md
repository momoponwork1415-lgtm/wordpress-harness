...
| Agent-led Research migration | `ResearchCampaigns.conduct / inspect`、sealed Native Run、継続・停止・incompleteの新event streamを実装 | Claude/Grok Agent Runtime、Independent Validation、Finding接続、旧経路削除（#143–#148） |
...
production v7のContext外入口は`openResearch`。移行中のagent-led入口は`openResearchCampaigns`であり、別schema familyを使う。旧経路を新経路のfallbackにせず、cutover後に#146で削除する。
...
- **Status:** scripted Native Agent Runtimeを使う最小vertical slice、同一`conduct`内の継続・停止、再起動後のinspect、run/time/cost Budget interruptionを実装。Claude Code Adapterはrunsc availability、digest-pinned image、sandbox内CLI versionをpreflightし、Target read-only / scratch write、capability drop、Bash / Web denialを固定したprocess requestからnative subagent数、usage、Research ReportをReceiptへ変換する。process contractはfake Dockerで検査済みだが、実image / providerを使うAcceptance、Grok Adapter、Independent Validation、Finding接続は未実施（[#143](https://github.com/momoponwork1415-lgtm/wordpress-harness/issues/143)、[#144](https://github.com/momoponwork1415-lgtm/wordpress-harness/issues/144)、[#148](https://github.com/momoponwork1415-lgtm/wordpress-harness/issues/148)）。
- **Code / Behavior Test:** [agent-led](../src/research/agent-led) · [ResearchCampaigns](../tests/research/research-campaigns.test.ts) · [Claude Code Adapter](../tests/research/claude-code-native-agent-runtime.test.ts)
...
- **Budget admission:** v7 PlanはSemantic Policy、全roleのmodel profile、prompt、source tool policy、Finder knowledge、Validation policy / baseline / public surface / exclusionをdigest固定してrun前に照合する。run startはRoot Evaluation用100,000 exploration token reservationをLedger互換なRoot Attempt identityとして記録し、最初にadmitされたwave Root Attemptだけがatomicにclaimできる。claim済みreservationへ向かう再試行wave Root Attemptはprotected creditを二重に消費しない。intentは両ordinalとも同じreservation idをbindしてLedger decodeとreplayを不変に保つが、claimは一度だけ書く。current Campaignではattempt count、wall timeまたはprovider costの残budgetが足りなければrun conflictではなく`budget-exhausted`を返し、Waveは`evaluation-incomplete`で終える。異なるreservation idやdepth評価intentのidはこれまでどおりfail closedにする。通常のAttemptはPlan最大使用量のreservationとintentを一transaction、terminal completionとreported usageのsettlementを一transactionで記録する。
- **Budget enforcement:** current CampaignはExplorationとValidationのwall time、provider costをowner partitionでadmitし、model attempt countもCampaign全体でadmitする。providerが返すtoken / turnはcache readを含み得るreported postconditionとしてLedger、overshoot、remainingへ観測記録するだけで、後続Attemptを拒否しない。legacy Campaignは元のtoken admission semanticsでreplayする。Explorationのreported overshootでValidation reserveを減らさず、owner / Campaign remainingを0未満にしない。
