...
上段はaccepted product path、下段はその能力を判定するevaluation gateである。legacy v5主経路はDepthと旧Independent Verificationまで、v6主経路は初回Semantic Wave、Decision@3、source-only Validation、Depth Work Queueまで接続済みで、v6 Depth実行 / Coverage ClosureとHuman OSは未実装である。接続済みを発見能力の証明とは数えず、Brizy、SSA、TranslatePressの各mechanismが新しい反復基準を満たすまではProspectiveへ進まない。

残作業は次の三群に分ける。

| 群 | 状態 | 次の有限work |
| --- | --- | --- |
| **いま閉じる** | v6初回WaveからDecision@3、Family Registry / Validation Ledger、source-only Validation、Disposition feedback、Depth Work Queueのdurable projectionまで接続済み。legacy v5は旧Verificationをreplay互換として維持する | v6 Root Synthesis / Critic / Depth Evaluation、Validation Frontier Gapの次Wave scheduling、二段Coverage Closure、Risk Assessment / Review Packet handoffを順に接続する |
