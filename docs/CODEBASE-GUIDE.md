...
**Failure semantics:** runsc、image、unprobed version、binding、Checkpoint integrity、policy、providerまたはschema failureをtyped terminal receiptへする。timeoutとproviderのbudget error envelopeは`budget-exhausted`である。providerがnon-zero exitしても、対応するerror envelopeのusage、costと有効なCheckpoint refを失わず、wall timeはprovider値とhost観測値の大きい方を記録する。GLM Researchのouter envelopeが正常でinner JSONだけ不正な場合は、同じCheckpointへ同一Reportの再符号化を一度だけ要求し、両attemptのusageを合算する。補正後も不正なら`invalid-output`を維持する。
