...
- **Owned artifacts:** Finding-bound Attempt、immutable execution Claim、append-only AI Verification Recordを専用current streamへCAS-firstで保存する。exact Reproduction Recipe、payload、request、screenshot、runtime logは専用private storeへ置き、public recordにはdigest検証済みopaque refだけを残す。Claimはcoordination recordであり、完了状態は同じAttemptのVerification eventから導出する。
- **Invariants:** AttemptへFinding、Target/version、Manifest、attacker premise、source route、Runtime Profile、Setup Plan、no-ambient-tool policyをbindする。experiment前にSQLite transactionでAttemptを一意にClaimし、同じAttemptの並行実行を拒否する。gVisor AdapterだけがEnvironment Builderのlive sessionでexperimentを実行し、sessionを外へ渡さない。AI outputはFindingを上書きせず、programme eligibilityを作らない。
- **Failure semantics:** setupとtyped provider / budget / unsupported / ambiguous outcomeはreason code付き`setup-blocked` / `inconclusive`として保持する。cleanup failureでも完了済みRecipe / evidenceは保持する。Environment Builder、CAS、event integrityまたは未知のHarness errorはVerification Recordへ丸めずthrowする。未知errorまたはprocess interruption後の未完了Claimは、次の`run`で開始時刻を持つ`FindingAIReproductionInProgressError`として明示し、自動再実行しない。外部experimentとSQLiteをatomicにcommitできないため、重複を避けるにはoperatorが先行実行をreconcileしてから別の回復変更を行う必要がある。`disproved`は前提一致かつRecipe完走後のSecurity Effect非観測だけに限定する。完了済みAttemptは保存済みRecordを返し、異なるcontentの占有はfail closedにする。
- **Status / Tests:** Finding-bound current contract、durable Claimによる並行実行拒否、idempotent completion replay / reopen、明示的interrupted Claim、dedicated SQLite stream、private bytes digest check、Environment Builder + typed gVisor experiment Adapterを実装。旧Runtime Verification Packet / Triage writerはcurrent barrelから外し、[#129](https://github.com/momoponwork1415-lgtm/wordpress-harness/issues/129)までdirect legacy moduleと既存decoderを保持する · [code](../src/human-os/ai-reproduction.ts), [gVisor adapter](../src/human-os/gvisor-ai-reproduction-harness.ts), [contracts](../src/human-os/ai-reproduction-contracts.ts), [behavior](../tests/human-os/finding-ai-reproduction.test.ts), [legacy replay](../tests/human-os/ai-reproduction.test.ts) · [#133](https://github.com/momoponwork1415-lgtm/wordpress-harness/issues/133)
