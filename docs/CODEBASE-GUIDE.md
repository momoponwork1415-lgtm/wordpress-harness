...
`CampaignRunner.run`の最初のbehavior sliceは、CAS固定のSurface Mapから有限Work Wave、Finder、独立Verification、Finding、Iteration Decision、Research Ledger replayまでを一つのpublic入口で接続している。Finderの外部実行より前にAttempt intentを記録し、中断したprocessを`orphaned`へ確定して残予算内のfresh Attemptへ置き換える。production Finder materializerはFocus AreaとWork Leaseごとに`Analysis Unit@v1`を作り、Strategy別に共有hook、PHP template参照、Map relation、call neighbor、広いSurface Map標本を上限内で選ぶ。directedな二次sourceからは、低頻度literal最大1件と共有hook登録元最大1件を一段だけ同じ上限内へ昇格し、`wildcard`は広い対照contextを維持する。Unitは実source digest、range、byte量、選択理由を固定したoracle-free promptの一部になる。仮説なしで予算が残る場合はpure Iteration Reviewが停止条件付きの有限next workをCASへ固定する。opaqueなprivate Calibration Reviewはterminal Verification refsだけを比較し、context digest付きreceiptが揃った場合だけ`stop-boundary-pair-complete`を記録する。閉路system testに加え、privateなBrizy Boundary Pairへproduction adapterを接続した。2.8.11のoracle-free Campaignは3 Opus Finderからsource-bound Hypothesis、独立Verifier、gVisor/browser experiment pair、Findingへ到達し、2.8.12のoracle-free対照は同じbounded context規則でFindingを作らなかった。

PHP Program Index 0.2.0は主要request superglobalと、SQL、file write、code/process executionの局所的なoperationをtyped factにする。Surface Mapはそれらをsource anchor付きnodeへ変えるが、taint edgeや脆弱性を決めない。Brizy 2.8.11では203 source、339 sink、Iptanus/WordPress File Upload 4.24.12では431 source、432 sinkを再現し、既知RCEのmiss分析で不足していた`$_COOKIE`と`require_once`を独立観測点として取得した。Finder materializerはこれらを開始点にbounded Analysis Unitを作る。Focus portfolioは公開Interfaceを変えず、外部entry、既知route、危険primitive、情報利得proxy、coverage debtの順で最初の3 Leaseを選び、feature diversityを維持する。
