...
| Model Execution | partial | Finder schema、共有Claude structured process、tool-free version/auth probe、budget/process-group終了、provider envelope policy監査、private Finder result CAS | [Model execution seam](design/model-execution-seam.md) |
...
現在の最優先delivery contractは[First closed vertical slice goal](design/first-closed-slice-goal.md)である。探索の水平拡張より先に、acceptedな[Campaign execution seam](design/campaign-execution-seam.md)と[Verification seam](design/verification-seam.md)でBrizy Boundary Pairを閉じる。production codeはone-Wave実行、crash recovery、Iteration Decision、Calibration Review receiptまで進んでいる。残るClosure Gateはprovider復旧後の同一Causal Identityによる2.8.12 Disprovedと、実レシートの`stop-boundary-pair-complete`である。
