...
| Bounded Source Evidence queries and Attempt binding | partial, internal; deterministic provider tracer only | `SourceEvidenceGateway.query` through `ModelExecution.run` | [`source-evidence-contracts.ts`](../src/research/source-mapping/source-evidence-contracts.ts), [`source-evidence-gateway.ts`](../src/research/source-mapping/source-evidence-gateway.ts), [`model-execution.ts`](../src/research/model-execution/model-execution.ts) | [`source-evidence-gateway.test.ts`](../tests/research/source-evidence-gateway.test.ts), [`source-evidence-attempt-loop.test.ts`](../tests/research/source-evidence-attempt-loop.test.ts) | [Evidence-guided Finder loop](design/evidence-guided-finder-loop.md), [ADR 0112](adr/0112-treat-analysis-units-as-seeds-for-bounded-source-retrieval.md) |
...
次のFinder loopのprovider非依存tracerとして、`ModelExecution.run`はSource Tool Policyを持つAttemptだけへ`search`と`read`を公開できる。model-visible requestへAttempt、Lease、Target Snapshot、Policy、query ordinalをharness側で付与し、manifest外path、digest不一致、truncation、query上限を型付きReceiptとしてprivate CASへ保存する。合成fixtureでは初期source外のwrapperを`search -> read`で取得してFinder outputへ到達する。公式Claude processへこのtoolを渡すnative bridge、production Campaignのmaterializer、`symbol`、`graph`はまだ未接続であり、上の実Target経路は引き続きtool-freeである。
...
| Model Execution | partial | Finder schema、provider-neutral structured role bridge、共有Claude process、tool-free version/auth probe、budget/process-group終了、provider envelope policy監査、bounded provider error detail、private Finder result CAS、Attempt-bound Source Evidence callbackとquery ceiling | [Model execution seam](design/model-execution-seam.md) |
