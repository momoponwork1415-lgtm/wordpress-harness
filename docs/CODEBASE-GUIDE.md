...
| 内容 | ソース | 現在の関係 |
| --- | --- | --- |
| base Prompt | [WP2Shell-derived](../prompts/wordpress-plugin-research-v7.md) / [Cloudflare-derived](../prompts/wordpress-plugin-research-cloudflare-v1.md) / [Cloudflare upstream-derived](../prompts/wordpress-plugin-research-cloudflare-upstream-v1.md) | `research-methods.ts`がcanonical method idと本文digestをbindする。WP2Shell-derived方式は公開原文の動的multi-agent、十分に異なる探索経路、複数round、遅い知見の交差、敵対的二重確認、Rootの反復統合を維持し、課題固有のpositive oracle、RCE / `/flag`強制、最低6時間だけを除く。Cloudflare由来の手順は別方式として混ぜない。CLIから選択した本文を渡し、sealed digestを検査する |
