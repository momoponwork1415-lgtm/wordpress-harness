...
| 内容 | ソース | 現在の関係 |
| --- | --- | --- |
| base Prompt | [WP2Shell-derived](../prompts/wordpress-plugin-research-v5.md) / [Cloudflare-derived](../prompts/wordpress-plugin-research-cloudflare-v1.md) / [Cloudflare upstream-derived](../prompts/wordpress-plugin-research-cloudflare-upstream-v1.md) | `research-methods.ts`がcanonical method idと本文digestをbindする。WP2Shell-derived方式はsource-supportedな経路のsemantic neighborhood、value-transformation ledger、authority transitionをRootのprivate scratchで扱う。upstream-derived方式はpinned vendorから生成した本文を含む。CLIから選択した本文を渡し、sealed digestを検査する |
