...
| 群 | 状態 | 次の有限work |
| --- | --- | --- |
| **いま閉じる** | Validationのstandalone coreは実装済み、Campaignは旧Verificationへ接続中 | Wave Barrier / Root Evaluation後のcandidate admission、Validation Ledger、Disposition feedbackをCampaignへvertical sliceで接続する |
