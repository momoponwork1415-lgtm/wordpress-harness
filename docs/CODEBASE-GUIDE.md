...
3. [アーキテクチャ概要](design/architecture/architecture-overview.md)の3枚の図で全体と信頼領域を確認し、探索を変更する場合は[自由探索エージェント・ループ](design/architecture/autonomous-research-loop.md)と[探索エージェント構成](design/architecture/exploration-agent-architecture.md)を読む。固定contextから予算付きsource追跡へ移るaccepted designは[Evidence-guided Finder loop](design/evidence-guided-finder-loop.md)を参照する。
4. 作業中のGitHub Issueから、変更するModuleのSeam文書とTestを一つずつ開く。

Source Mappingを変更する場合は、先に[攻撃面マップ構成](design/architecture/surface-map-architecture.md)で「小さなInterface」「根拠状態」「AI Mapperとの責任分担」を確認する。
...
| Bounded Source Evidence queries and Attempt binding | partial, internal; deterministic and native Claude bridge implemented | `SourceEvidenceGateway.query` through `ModelExecution.run` | [`source-evidence-contracts.ts`](../src/research/source-mapping/source-evidence-contracts.ts), [`source-evidence-gateway.ts`](../src/research/source-mapping/source-evidence-gateway.ts), [`model-execution.ts`](../src/research/model-execution/model-execution.ts), [`claude-source-evidence-bridge.ts`](../src/research/model-execution/claude-source-evidence-bridge.ts) | [`source-evidence-gateway.test.ts`](../tests/research/source-evidence-gateway.test.ts), [`source-evidence-attempt-loop.test.ts`](../tests/research/source-evidence-attempt-loop.test.ts), [`claude-source-evidence-bridge.test.ts`](../tests/research/claude-source-evidence-bridge.test.ts) | [Evidence-guided Finder loop](design/evidence-guided-finder-loop.md), [ADR 0112](adr/0112-treat-analysis-units-as-seeds-for-bounded-source-retrieval.md) |
...
| Restricted Claude Finder and structured role execution | partial, internal; optional Source Evidence MCP and Route Fragment terminal output implemented | `ModelExecution.run` / `StructuredModelExecution.run` | [`model-execution/index.ts`](../src/research/model-execution/index.ts), [`model-execution.ts`](../src/research/model-execution/model-execution.ts), [`claude-process.ts`](../src/research/model-execution/claude-process.ts) | [`model-execution.test.ts`](../tests/research/model-execution.test.ts), [`structured-model-execution.test.ts`](../tests/research/structured-model-execution.test.ts), [`claude-source-evidence-bridge.test.ts`](../tests/research/claude-source-evidence-bridge.test.ts) | [Model execution seam](design/model-execution-seam.md) |
...
Surface Map
  ...
  -> schema-valid Finder Attempt Results
     -> complete Hypotheses
     -> partial Route Fragment Proposals
...
`CampaignRunner.run`は、CAS固定のSurface Mapから有限Work Wave、最大3並列Finder、独立Verification、Finding、Iteration Decision、Research Ledger replayまでを一つのpublic入口で接続している。production Finder materializerはLeaseごとに`Analysis Unit@v1`を作り、同じsink family、Map/call近傍、class-like symbol、低頻度literal、共有hook、広いSurface Map標本を上限内で選ぶ。Unitは実source digest、range、byte量、選択理由を固定したoracle-free promptの一部になる。全Finderの`provider-failed`は仮説0件へ丸めず`provider-unavailable`になる。Brizy Stored XSSではoracle-free positive、独立Verifier、fresh gVisor Witness/Control、Finding、同じCausal Identityのpatched Disprovedを公開経路に通した。Appointment Booking Calendar SQLiはroot mechanismをFindingにしたが、完全な未認証premiseと同一Identityのpatched Disprovedは未完了である。

これは現在のMap-first実装経路であり、到達設計ではない。最新方針はraw-source first、自由なApproach Family、Root Synthesis、Adversarial Critic、Missing-link Waveであり、[調査設計原則](design/research-design-principles.md)と[ハーネス完成度監査](design/harness-completeness-audit.md)を正本とする。公開CVEを使った実runの時系列は[2026-09-02 E2E実験結果](experiments/e2e-campaign-results-2026-09-02.md)を参照する。

Finder loopでは、`Campaign Run Plan`がTarget-boundなSource Tool Policy refとAttempt query ceilingを固定し、production Finder materializerが対応するPolicy本文、digest、Target bindingを検査して`Attempt Plan`へ結合する。`ModelExecution.run`はそのAttemptだけへ`search`と`read`を公開し、model-visible requestへAttempt、Lease、Target Snapshot、Policy、query ordinalをharness側で付与する。manifest外path、digest不一致、truncation、query上限は型付きReceiptとしてprivate CASへ保存する。公式Claude processはAttempt-localなprivate MCP configからloopback Source Evidence bridgeだけを読み、provider組込みtoolを空にしてexact allowlistを`dontAsk`で実行する。合成fake providerのCampaign binding、`search -> read`、Claude bridge Behavior Testと、subscription認証済みClaude Code 2.1.258のlive capability probeは通過した。実Targetのtool付き3並列CampaignもBrizy、Appointment Booking Calendar、TranslatePressで実行済みである。Finderは完全なHypothesisに届かないsecurity-relevant primitiveを、攻撃者前提、値、state transition、source位置、unknown、falsifier付きの`Route Fragment Proposal`としてAttempt Resultとprivate CASへ残せる。現時点ではProposalのWave取込、Target inventoryとの再検査、Chain Synthesisは未実装である。`symbol`と`graph`も次の検証対象である。
...
| Source Understanding | partial | PHP Program Index、静的initial/source revision、Context Response、relation-only Map Delta compile/Receipt、model failure gap、asset inventory、根拠状態、manifest拘束したexact `read/search` | [Source mapping seam](design/source-mapping-seam.md) |
| Exploration | partial | bootstrap Map gate、route-aware Focus portfolio、有限Work Wave、Lane/Strategy/model-family割当、Source-bound Hypothesis取込。Route Fragment schemaは存在するがWave取込は未実装 | [Exploration seam](design/exploration-seam.md) |
| Verification | partial | `verify(plan)`、bounded source re-derivation、Stored XSS/SQLi typed Experiment、共通gVisor WordPress Lab、typed Witness/Control、durable replay。Brizy Boundary Pairは完了、SSAはroot mechanism Findingまで | [Verification seam](design/verification-seam.md) |
| Model Execution | partial | Finder Hypothesis/Route Fragment schema、provider-neutral structured role bridge、共有Claude process、version/auth probe、budget/process-group終了、provider envelope policy監査、bounded provider error detail、private Finder result CAS、Attempt-bound Source Evidence callbackとquery ceiling、private loopback Claude MCP bridge | [Model execution seam](design/model-execution-seam.md) |
