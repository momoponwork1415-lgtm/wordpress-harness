...
- **Invariants:** AttemptへFinding、Target/version、Manifest、attacker premise、source route、Runtime Profile、Setup Plan、no-ambient-tool policyをbindする。gVisor AdapterだけがEnvironment Builderのlive sessionを所有し、AI Brokerへはisolated WordPress originに対するbounded HTTP exchangeだけを渡す。image、command、environment、host mount、sessionは渡さない。AI outputはFindingを上書きせず、programme eligibilityを作らない。
