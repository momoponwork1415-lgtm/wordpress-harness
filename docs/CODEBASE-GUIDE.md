...
- **Invariants:**
  ...
  - **Index and raw artifacts:** production indexはDatabase constructorより前にowner-only parentとDB / WAL / SHMを`O_NOFOLLOW`で検証し、open前後のdevice / inodeをcached instanceへbindする。`run / inspect`はindexとraw CAS directoryのowner、mode、file type、real pathを毎回再検証し、driftやpath replacementを修復しない。raw CASはpinned directory descriptor配下の同期済みowned tempからatomic no-clobber installし、winnerをowner、regular file、mode `0400`、全bytesへ照合してparentをfsyncしてからSQLite参照をcommitする。byte-identicalで検証済みの同一path replacementは許可する。
  ...
  - **Refresh state and ordering:** success publication、failure state、current bindingはSQLite immediate transactionで直列化し、`inspect`は一つのread snapshotから投影する。active Attemptはsequence付きで個別に保存し、completionはexact token membershipを検証して自分だけをfinalizeする。modern orderは`publication <= latest-completed <= allocated`かつ`allocated = max(latest-completed, active sequences)`を満たし、`latest-completed > publication`なら同じsequenceのv2 failure stateを必須とする。orderなしで初期化できるのはactive / v2 stateのないclean stateまたはsequence-free legacy（current-onlyまたはv1 state）だけである。
