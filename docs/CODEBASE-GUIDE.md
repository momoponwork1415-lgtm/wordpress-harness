...
- **Invariants:** callerはFinder、role、Wave、Lease、Depth、CWE、fileまたはsubagent数を指定しない。AIの`continue`は保存済みreport履歴とともに次のsealed runへ進み、`stop`はactionable frontierが残らない根拠を保持する。max native runs、reported wall time、reported provider costは次run開始前に強制する。
...
- **Status:** scripted Native Agent Runtimeを使う最小vertical slice、同一`conduct`内の継続・停止、再起動後のinspect、run/time/cost Budget interruptionを実装。Claude Code / Grok production Adapter、Permission ProfileのOS enforcement、Independent Validation、Finding / Coverage分離は未接続（[#143](https://github.com/momoponwork1415-lgtm/wordpress-harness/issues/143)、[#144](https://github.com/momoponwork1415-lgtm/wordpress-harness/issues/144)、[#148](https://github.com/momoponwork1415-lgtm/wordpress-harness/issues/148)）。
- **Code / Behavior Test:** [agent-led](../src/research/agent-led) · [ResearchCampaigns](../tests/research/research-campaigns.test.ts)
