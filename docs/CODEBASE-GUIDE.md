...
flowchart LR
    ...
    research["Semantic Research<br/>Map-free v6接続済み<br/>conditional Depthはlegacyのみ"]
    validation["Source-only Validation core<br/>v6 Campaign接続済み<br/>cohort未合格"]
...
上段はaccepted product path、下段はその能力を判定するevaluation gateである。legacy v5主経路はDepthと旧Independent Verificationまで、v6主経路は初回Semantic Wave、Decision@3、source-only Validationまで接続済みで、v6 Depth / Coverage ClosureとHuman OSは未実装である。接続済みを発見能力の証明とは数えず、Brizy、SSA、TranslatePressの各mechanismが新しい反復基準を満たすまではProspectiveへ進まない。

残作業は次の三群に分ける。

| 群 | 状態 | 次の有限work |
| --- | --- | --- |
| **いま閉じる** | v6初回WaveからDecision@3、Family Registry / Validation Ledger、source-only Validation、Disposition feedbackまで接続済み。legacy v5は旧Verificationをreplay互換として維持する | v6 conditional Depth、Validation Frontier Gapの次Wave scheduling、二段Coverage Closure、Risk Assessment / Review Packet handoffを順に接続する |
...
二段Coverage Closureは旧production sliceへ接続済みである。初回complete WaveまたはterminalなDepth後の評価を一回目の観測とし、一回のno-material-deltaだけでは閉じない。後段は既知subjectやclosure hintをassignmentへ渡さないfresh Wildcard Finderとfresh Root Evaluatorを起動する。現行Campaign codeはactive / blocked Family、pending legacy Verification、未解決workがない時だけ`coverage-closed`を作る。新policyではこれをpending Validationへ置き換え、Human DeferredまたはHuman VerificationをResearch closure条件にしない。直近の実装gapはv6 conditional Depth、Validation Frontier Gapの次Wave scheduling、二段Coverage Closure、Risk Assessment / Review Packet handoffである。Map、PHP Program Index、AST、Semgrep、CodeQLは補助に残し、Map外candidateを拒否しない。

`semantic-research-recall-baseline-v5`は旧Verificationへ接続したlegacy runtimeのversioned resource policyであり、新規production pathは`semantic-research-recall-baseline-v6` Campaign Planから初回Semantic Wave、Decision@3、source-only Validation、DispositionのFamily反映まで接続済みである。v6はLab BaselineとExperiment registryを持たず、Opus Validator / Validation Synthesis、source-only tool、Validation PolicyとUSD 30のValidation専用reserveを固定する。Finderは512 source query、16 GiB scan、256 MiB response、256 turn、USD 20、2 MiB output、3 hours、Reconは256 query、128 turn、USD 10、1 hour、Root Evaluationは128 turn、USD 10、1 hourを遠いemergency envelopeとして持つ。Campaign全体は12 Wave、48 Finder Attempt、4 concurrent Finder、128 model Attempt、USD 150、12 hoursである。Validationを低いcandidate件数またはtokenだけで打ち切らず、累積provider costとwall timeを残り予約へ反映する。source query / byte、wall time、provider cost、output byteはhard guardrailとして残し、ceiling到達をnegativeへ丸めない。Brizy、Simply Schedule Appointments、TranslatePressのv6 PlanはmodelやHuman Verification Environmentを起動しないpreflight testを通す。旧budget schemaは完了済みRunのreplay用にdecodeする。v6のconditional Depth、二段Coverage Closure、Risk Assessment / Review Packet handoffは未接続で、残workを`research-work-remains`としてterminal Runへ明示する。
...
Manual local-directory Target IntakeからCampaign preparationへのversioned context handoffまで実装済みである。Source-only Validationはv6初回Waveから接続済みである。Target選定、archive acquisition、Human Review Packet、Human Verification EnvironmentとHuman OSは未実装である。
...
| Campaign prepare、run、replay | partial; Default Map-free Semantic Wave E2E、legacy v5実行、Validation-only v6 Planの3-target preflight、初回WaveからDecision@3 / source-only Validation / v3 terminal replayまで接続、v2 Manifest-bound prepare、v1/v2/v3 replay、Ledger-derived live progress。v6 conditional Depth、二段Closure、Review Packetは未接続 | `openResearch` / `CampaignRunner` / `CampaignReader.inspect(progress)` | [`campaign-control/`](../src/research/campaign-control), [`campaign-progress-reporter.ts`](../src/research/campaign-progress-reporter.ts), [`target-file-manifest.ts`](../src/research/source-mapping/target-file-manifest.ts), [`open-research.ts`](../src/research/open-research.ts) | [`v6 Validation run`](../tests/research/campaign-validation-run.test.ts), [`semantic E2E`](../tests/research/campaign-semantic-e2e.test.ts), [`v6 budget dry-check`](../tests/research/semantic-recall-budget.test.ts), [`campaign-prepare`](../tests/research/campaign-prepare.test.ts), [`semantic run`](../tests/research/campaign-semantic-run.test.ts), [`campaign-run`](../tests/research/campaign-run.test.ts), [`progress reporter`](../tests/research/campaign-progress-reporter.test.ts) | [Campaign seam](design/campaign-execution-seam.md) |
| Research Ledger and CAS | partial; Finder checkpoint event、close/reopen replay、Ledger / CAS prefixからの決定的progress projection、Iteration Decision@3と初期Family Registry v3のCAS-first単一event記録、dedupe済みValidation Intentと全origin Family pending状態、Validation completionとFamily outcomeのCAS-first記録、Ledger-only replayを実装済み。`validation-pending`はpendingを解消せず保持し、`needs-research`はSynthesisが選んだproof gapだけを同じFamilyへbindしたValidation Frontier GapとしてCASとLedgerへ残す。v6 Campaign Runnerはこれらを利用し、Decision、全Validation completion、最終Family Registry、Frontier Gapをv3 terminal Runへbindして再生する | internal `ResearchRecord` | [`research-record/`](../src/research/research-record) | [`v6 Validation run`](../tests/research/campaign-validation-run.test.ts), [`campaign run`](../tests/research/campaign-run.test.ts), [`semantic run`](../tests/research/campaign-semantic-run.test.ts), [`Decision / Validation record v3`](../tests/research/semantic-iteration-decision-record-v3.test.ts), [`ledger compatibility`](../tests/research/ledger-compatibility.test.ts) | [Harness Architecture](ARCHITECTURE.md) |
...
| Planning and Hypothesis intake | partial; Source-aware Reconとwhole-target Baselineの並行開始、source-backed Focus Packet、Manifest-bound Wave Barrier、checkpoint Hypothesisの先行legacy Verification、fresh Root Evaluation、atomic Iteration Decision、active Approach Family Registry、全Depth Admission / next workを失わないversioned Depth Work Queue、tool-freeなfresh Root SynthesisとManifest-bound Chain Proposal、fresh source readを必須にするAdversarial Critique、fresh Depth Root Evaluation、Family未接続Proposalのmodel-owned genesis、Gap-boundなMissing-link Wave、Family evidence attachment、Familyごと最大3世代かつ全体最大12 Waveのfresh反復、容量超過Gapのtyped incomplete、Depth Chain ProposalからSource-bound Hypothesisへの変換、Verification outcomeのFamily feedback、fresh Wildcardによる二段Coverage Closureをlegacy Campaign E2Eで実装済み。現行Root Evaluation@3はValidation/Depth actionをexplicitな同一Family Admissionへbindし、Decision単体からCampaign-local Registry v3を決定論的に投影する。同じAdmissionを両actionが参照しても一Familyだけを開き、Validation Candidate originと同じFamily IDへ収束する。Research RecordのCAS-first記録とLedger replay、およびv6初回WaveからDecision@3 / ValidationまでのCampaign接続は実装済み。v6のDepthとClosureへの接続は未実装 | `Exploration.decide` / `SemanticChainSynthesis.synthesize` / `SemanticAdversarialCritique.critique` / `CampaignRunner.run` | [`exploration/`](../src/research/exploration), [`campaign-control/`](../src/research/campaign-control) | [`semantic E2E`](../tests/research/campaign-semantic-e2e.test.ts), [`Family transition`](../tests/research/semantic-approach-family-transition.test.ts), [`Depth work queue`](../tests/research/semantic-depth-work-queue.test.ts), [`Root Synthesis`](../tests/research/semantic-chain-synthesis.test.ts), [`Adversarial Critic`](../tests/research/semantic-adversarial-critique.test.ts), [`semantic-root-planning`](../tests/research/semantic-root-planning.test.ts), [`root evaluation`](../tests/research/semantic-root-evaluation.test.ts), [`Candidate admission`](../tests/research/validation-candidate-admission.test.ts), [`Decision v3 record`](../tests/research/semantic-iteration-decision-record-v3.test.ts), [`semantic run`](../tests/research/campaign-semantic-run.test.ts), [`wave barrier`](../tests/research/semantic-wave-barrier.test.ts), [`exploration-bootstrap`](../tests/research/exploration-bootstrap.test.ts) | [Exploration seam](design/exploration-seam.md) |
...
| Source-only Validation | partial; exact semantic candidate identity、2 fresh Validator、material rubric conflict時だけ第三Attempt、各model resultの次stage前CAS固定、tool-free Synthesis、evidence ref検査、4 terminal Dispositionとfailure時Validation-pendingを`Validation.validate`で実装済み。Decision@3のmodel-owned ordered routeをHypothesis anchorへ検査し、exact duplicateの全Campaign-local Family originを保持するCandidate materializer、Candidate CASとdedupe済みValidation Intent、Family pending/outcome projection、`validation-pending` completion replayを実装済み。`needs-research`は選択されたproof gapを同じFamilyのValidation Frontier Gapとして記録・再生する。v6 CampaignRunnerからのValidation起動とFrontier Gap記録は接続済み。Frontier Gapの次Wave scheduling、Risk Assessment、Review Packetは未接続 | `Validation.validate` | [`validation/`](../src/research/validation), [`validation-candidate-admission.ts`](../src/research/campaign-control/validation-candidate-admission.ts), [`research-record/`](../src/research/research-record) | [`Validation`](../tests/research/validation.test.ts), [`Candidate admission`](../tests/research/validation-candidate-admission.test.ts), [`Validation record`](../tests/research/semantic-iteration-decision-record-v3.test.ts), [`v6 Validation run`](../tests/research/campaign-validation-run.test.ts) | [Validation seam](design/validation-seam.md) |
...
| Target Selection / Human OS | planned | not implemented | — | — | [Harness Architecture](ARCHITECTURE.md) |

## Fifteen-minute reading path

1. [Documentation](README.md)で目的別の入口を選ぶ。
2. [Harness Architecture](ARCHITECTURE.md)でownerを確認する。
...
| 安定したModule関係 | [Harness Architecture](ARCHITECTURE.md) |
