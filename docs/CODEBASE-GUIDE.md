...
| Source-only Validation | partial; exact semantic candidate identity、2 fresh Validator、material rubric conflict時だけ第三Attempt、各model resultの次stage前CAS固定、tool-free Synthesis、evidence ref検査、4 terminal Dispositionとfailure時Validation-pendingをstandalone `Validation.validate`で実装済み。Decision@3のmodel-owned ordered routeをHypothesis anchorへ検査し、exact duplicateの全Campaign-local Family originを保持するCandidate materializer、Candidate CASとdedupe済みValidation Intent、Family pending/outcome projection、`validation-pending` completion replayを実装済み。`needs-research`は選択されたproof gapを同じFamilyのValidation Frontier Gapとして記録・再生する。CampaignRunnerからのValidation起動とFrontier Gap scheduling、Risk Assessment、Review Packetは未接続 | `Validation.validate` | [`validation/`](../src/research/validation), [`validation-candidate-admission.ts`](../src/research/campaign-control/validation-candidate-admission.ts), [`research-record/`](../src/research/research-record) | [`Validation`](../tests/research/validation.test.ts), [`Candidate admission`](../tests/research/validation-candidate-admission.test.ts), [`Validation record`](../tests/research/semantic-iteration-decision-record-v3.test.ts) | [Validation seam](design/validation-seam.md) |
