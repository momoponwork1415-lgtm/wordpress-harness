...
- **所有する記録・不変条件:** SQLiteのappend-only Campaign eventsとprivate Checkpoint / Diagnostic / Candidate Recipeへのopaque ref。Campaign outcome view v3はprovider呼び出し前にexact Sealed Native RunへbindしたNative Run Attemptを公開し、private Receipt artifactのatomic確定後だけterminal eventを記録する。`researchProgress`はcompleted Grantのexact `evidenceSummary`、観測したsource pathごとのrun provenanceと現在承認待ちのnext actionを同じ記録からread-onlyに導出し、failed / orphaned runを探索済みへ変換しない。Research Methodは`promptSet.id + digest`としてCampaign、runとCheckpointへbindし、canonicalな`wp2shell` / `cloudflare` / `cloudflare-upstream`のIDと本文digest対応を[`research-methods.ts`](../src/research/agent-led/research-methods.ts)で固定する。`cloudflare-upstream`はpinned公開skillをMethod内部のprivate scratch workflowとして使い、そのcoverage ledgerをHarness stateやResearch Coverageへ昇格させない。方式ごとのstate、schedulerまたはAdapterを追加しない。Research Report v2はGrant内のexamined / unexamined領域を示すsource-backed evidence summary、Grant-localなResearch Assessmentと、ordered source trace・control assessment・未解決事実を持つCandidateを記録する。completed Native Run Receiptと、その結果をCampaignへ採用できないResearch Admission Failureは別eventとして同じtransactionで記録する。最大1時間のGrant、exact bindingの両Human Review、Parked Programme LeadのCandidate Review除外、admitされたCandidateだけのRequest生成、Research Coverageとの分離を維持する。`researchProgress`、evidence summaryやAssessmentをwork queue、探索完了またはCoverage proofにせず、Candidate VerificationへCheckpointやProgramme Boundaryを渡さない。run数とwall timeがhard limit、provider costは観測値。
...
**Source / Behavior Test:** [`research-campaigns.ts`](../src/research/agent-led/research-campaigns.ts) · [`research-methods.ts`](../src/research/agent-led/research-methods.ts) · [`native-run-receipts.ts`](../src/research/agent-led/native-run-receipts.ts) · [`research-campaigns.test.ts`](../tests/research/research-campaigns.test.ts) · [`research-methods.test.ts`](../tests/research/research-methods.test.ts) · [`human-research-continuation-review.test.ts`](../tests/research/human-research-continuation-review.test.ts) · [`candidate-verification-handoff.test.ts`](../tests/research/candidate-verification-handoff.test.ts) · [`parked-programme-leads.test.ts`](../tests/research/parked-programme-leads.test.ts) · [`agent-led-campaign-cli.test.ts`](../tests/cli/agent-led-campaign-cli.test.ts)
...
| 内容 | Source | 現在の関係 |
| --- | --- | --- |
| base Prompt | [WP2Shell-derived](../prompts/wordpress-plugin-research-v3.md) / [Cloudflare-derived](../prompts/wordpress-plugin-research-cloudflare-v1.md) / [Cloudflare upstream-derived](../prompts/wordpress-plugin-research-cloudflare-upstream-v1.md) | `research-methods.ts`がcanonical method idと本文digestをbindする。upstream-derived方式はpinned vendorから生成した本文を含む。CLIから選択した本文を渡し、sealed digestを検査する |
| run固有のcontextと追加指示 | [`agent-prompts.ts`](../src/research/agent-led/agent-prompts.ts) | `agentResearchPrompt`がsealed runからTarget・Dependency・Threat Context・Programme Boundary・継続next actionの表示とJSON化したcontextを組み立て、Candidate-bound recipe、Candidate evidence、Grant-local Research Assessmentとevidence summaryの出力を要求する。Continuationでは復元したCheckpoint / scratch履歴を先に参照し、承認next action、新証拠、Candidate validation gapまたは具体的なchainがない完了routeの機械的な再探索を避ける |
