...
PHP Program Index 0.2.0は主要request superglobalと、SQL、file write、code/process executionの局所的なoperationをtyped factにする。Surface Mapはそれらをsource anchor付きnodeへ変えるが、taint edgeや脆弱性を決めない。Brizy 2.8.11では203 source、339 sink、Iptanus/WordPress File Upload 4.24.12では431 source、432 sinkを再現し、既知RCEのmiss分析で不足していた`$_COOKIE`と`require_once`を独立観測点として取得した。Finder materializerはこれらを開始点にbounded Analysis Unitを作る。次のcorrection sliceは公開Interfaceを変えず、最初の3 Leaseへ入れるFocusの探索価値順位を改善する。
