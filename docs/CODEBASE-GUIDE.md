...
| Research | AI判断によるCheckpoint自動継続、Human Candidate Review、Candidate Verification Request / Coverage表示 | `inspect`で進捗、Requestとrecipe準備不足を取得できる。Coverageは同じResearch Recordからread-onlyに導出し、別のmutable ledgerを持たない |
...
- **所有する記録・不変条件:** SQLiteのappend-only Campaign eventsとprivate Checkpoint / Diagnostic / Candidate Recipeへのopaque ref。Campaign outcome view v4はprovider呼び出し前にexact Sealed Native RunへbindしたNative Run Attemptを公開し、private Receipt artifactのatomic確定後だけterminal eventを記録する。Rootがsource-boundな`continue`を返すと、completed runのexact Checkpointとnext actionを次runへbindして人間の介入なしに継続する。`stop`後だけHuman Candidate ReviewまたはResearch Progressの最終表示へ進む。`researchProgress`はcompleted runのexact `evidenceSummary`、観測したsource pathごとのrun provenanceと未完了のnext actionを同じ記録からread-onlyに導出し、failed / orphaned runを探索済みへ変換しない。Production ResearchのPrompt Setはcanonicalなwp2shell由来のIDと本文digest一組だけを[`research-prompt-set.ts`](../src/research/agent-led/research-prompt-set.ts)で固定し、Campaign、run、Checkpointへbindする。未知のIDまたはdigest不一致はprovider呼び出し前に拒否し、方式選択のstate、scheduler、Adapterを持たない。Research Report v2はrun内のexamined / unexamined領域を示すsource-backed evidence summary、run-localなResearch Assessmentと、ordered source trace・control assessment・未解決事実を持つCandidateを記録する。completed Native Run Receiptと、その結果をCampaignへ採用できないResearch Admission Failureは別eventとして同じtransactionで記録する。固定1時間capやrunごとの人間確認を持たず、Campaign全体のrun数・wall timeだけをsafety envelopeとして維持する。Parked Programme LeadをHuman Candidate Reviewから除外し、admitされたCandidateだけのRequest生成と、Candidateとは独立したread-onlyなResearch Progressを維持する。`researchProgress`、evidence summaryやAssessmentをwork queue、探索完了またはCoverage proofにせず、Candidate VerificationへCheckpointやProgramme Boundaryを渡さない。provider costは観測値。
...
**ソース / 振る舞いテスト:** [`research-campaigns.ts`](../src/research/agent-led/research-campaigns.ts) · [`research-prompt-set.ts`](../src/research/agent-led/research-prompt-set.ts) · [`native-run-receipts.ts`](../src/research/agent-led/native-run-receipts.ts) · [`autonomous-research-continuation.test.ts`](../tests/research/autonomous-research-continuation.test.ts) · [`research-campaigns.test.ts`](../tests/research/research-campaigns.test.ts) · [`research-prompt-set.test.ts`](../tests/research/research-prompt-set.test.ts) · [`candidate-verification-handoff.test.ts`](../tests/research/candidate-verification-handoff.test.ts) · [`parked-programme-leads.test.ts`](../tests/research/parked-programme-leads.test.ts) · [`agent-led-campaign-cli.test.ts`](../tests/cli/agent-led-campaign-cli.test.ts)
...
| 認証ファイルとprivate環境の検査・一時配置・削除・ログ秘匿 | [`ProviderCredentialFiles.copyTo / stageEnvironmentSettings / removeFrom / redact`](../src/research/agent-led/provider-files.ts) |
...
- **目的:** 各試行を以前の結果やCheckpointを持たない通常のResearch Campaignとして起動し、同一条件の独立Trialだけをread-only comparisonへ渡す。既知事例の`pass@3`は3 Trialを最初から起動する。未知対象の本番探索は初回だけを起動し、モデル上完了した初回にCandidateが一件以上あった時だけ残り2 Trialを並列起動する。
- **インターフェース:** `defineIndependentResearchTrialApproval`、`observeIndependentResearchTrialGate`、`decideIndependentResearchTrialLaunches`、`dispatchIndependentResearchTrials`、`inspectIndependentResearchTrialClaims`、installed CLI `wordpress-harness-trials`の`seal-approval / dispatch / inspect`。Approval v3はexactなTrial順、`all-approved`または`candidate-gated-followups`の起動方針、`approved-target-campaign-request`、source path、prompt、runtime profile / model、permission、各Trial予算、aggregate native-run / wall-time allowance、最大同時Trial数と期限を自己digestへbindする。本番方針はexactな3 Trialと初回IDを要求する。初回結果から作るgate observationと後続claimをdigestで結び付ける。Target Intelligenceのadmissionから比較bindingとCampaign Inputを導出し、起動は既存`campaign conduct-approved`だけを使う。
- **所有する記録・不変条件:** private receipt rootのimmutable claim、gate observationのdigest、launch receiptだけを所有し、Campaign stateは所有しない。全Trialは同じcomparison bindingを持ち、Campaign ID、database、scratch、logはTrialごとに一意、`resumeFrom`は禁止する。一つのprovider configに対するfreshなbalance observationと、Target / Dependency source、prompt、credential fileを検査する。Docker、runsc、pinned image、sandbox内`dsh --version`はCampaign Doctorと同じ`GvisorRuntimePreflight`でread-onlyに検査する。global dispatch lock内でclaimを再読込し、各Trialの全予算をaggregate allowanceへ予約してからfresh database / scratch / logを排他的に確保する。本番の後続2 Trialは初回結果の本文を受け取らず、起動根拠となるgate observationのdigestだけをclaimへ持つ。Approvalから作るcomparison requestは承認順のCampaign IDをそのまま使う。
- **失敗時:** raw Campaign Input、旧Approval、Target Intelligenceでadmitできないrequest、改変・期限切れApproval、stale / unauthenticated / exhausted / rate-limited account observation、binding不一致、blocked / unknown preflight、同時実行上限、aggregate allowance不足をfail closedにする。本番方針では初回の実行中、結果欠損、`incomplete`、Candidateなしを区別し、前3者をCandidateなしへ丸めない。Candidateなしなら後続を起動しない。gateと一致しない後続claim、初回より先の後続claimを拒否する。lock競合時は起動せず、claim後のspawnまたはreceipt保存失敗はclaimをactive-unknownのまま残す。中断claimを自動削除・再実行せず、Campaign内の正常なAI継続以外のretry、Candidate admission、Verificationを行わない。

**ソース / 振る舞いテスト:** [`independent-research-trial-launcher.ts`](../src/operations/independent-research-trial-launcher.ts) · [`independent-research-trial-launcher-runtime.ts`](../src/operations/independent-research-trial-launcher-runtime.ts) · [`independent-research-trial-readiness.ts`](../src/operations/independent-research-trial-readiness.ts) · [`gvisor-runtime-preflight.ts`](../src/infrastructure/gvisor-runtime-preflight.ts) · [`independent-research-trial-launcher-cli.ts`](../src/operations/independent-research-trial-launcher-cli.ts) · [`independent-research-trial-launcher.test.ts`](../tests/operations/independent-research-trial-launcher.test.ts)
...
**ソース / 振る舞いテスト:** [`campaign-readiness-doctor.ts`](../src/operations/campaign-readiness-doctor.ts) · [`gvisor-runtime-preflight.ts`](../src/infrastructure/gvisor-runtime-preflight.ts) · [`campaign-readiness-doctor-cli.ts`](../src/operations/campaign-readiness-doctor-cli.ts) · [`campaign-readiness-doctor.test.ts`](../tests/operations/campaign-readiness-doctor.test.ts) · [`gvisor-runtime-preflight.test.ts`](../tests/infrastructure/gvisor-runtime-preflight.test.ts)
...
| 内容 | ソース | 現在の関係 |
| --- | --- | --- |
| base Prompt | [WP2Shell-derived](../prompts/wordpress-plugin-research-v8.md) | `research-prompt-set.ts`が唯一のcanonical Prompt Set IDと本文digestをbindする。v2規模かつ6,500 bytes未満の本文で、公開原文の動的multi-agent、十分に異なる探索経路、複数round、遅い知見の交差、敵対的二重確認、Rootの反復統合を維持し、課題固有のpositive oracle、RCE / `/flag`強制、最低6時間だけを除く。証拠形式と非公開recipeはrun固有のPromptへ置き、CLIは正本本文を渡してsealed digestを検査する |
