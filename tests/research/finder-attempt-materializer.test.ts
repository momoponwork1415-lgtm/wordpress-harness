...
async function fixture() {
  ...
  const files = {
    ...
    "views/review-view.php": "<?php echo $persisted_value;\n",
  } as const;
  await Promise.all(
  ...
  const map: SurfaceMap = {
    ...
    relations: [],
    gaps: [],
    summary: { files: 4, nodes: 1, relations: 0, gaps: 0 },
  };
  ...
  const focusArea = {
    ...
    risk: { tier: "elevated" as const, basis: "registered-hook" as const },
  };
  const lease = {
    ...
    lane: "frontier" as const,
    strategy: "entry-forward" as const,
